<script lang="ts">
  import { generateZoomLevelsWithLabels } from './chart/zoomLevels';
  ...
  const zoomLevelsWithLabels = $derived(timeBetweenPoints && segmentDuration ? generateZoomLevelsWithLabels(timeBetweenPoints, segmentDuration) : []);
  ...
  const currentLevelIndex = $derived(selectedZoomLevel !== undefined ? zoomLevelsWithLabels.findIndex((level) => level.value === selectedZoomLevel) : -1);
...
<div class="bg-white p-4 rounded-lg shadow-md min-w-[140px] max-w-[180px] sm:min-w-[120px]">
  ...
  <div class="flex gap-2 mb-4">
    <button class="zoom-btn-in" disabled={!canZoomIn} onclick={handleZoomIn} title="Zoom In (decrease time span)">
      <div class="w-4 h-4">
        <ZoomIn />
      </div>
      In
    ...
    <button class="zoom-btn-out" disabled={!canZoomOut} onclick={handleZoomOut} title="Zoom Out (increase time span)">
      <div class="w-4 h-4">
        <ZoomOut />
      </div>
      Out
  ...
  <!-- Zoom Level Dropdown -->
  <div>
    <label for="zoomLevelSelect" class="block text-xs font-medium text-gray-700 mb-1"> Time Span: </label>
    <select id="zoomLevelSelect" class="form-select text-sm w-full sm:text-[10px]" value={selectedZoomLevel} onchange={handleDropdownChange}>
  ...
  {#if zoomLevelsWithLabels.length === 0}
    <div class="text-xs text-gray-500 mt-2">No zoom levels available</div>
  {/if}
</div>
