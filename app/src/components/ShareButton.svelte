...
<Modal bind:show={showModal} title="Share Dataset" onclose={closeModal}>
  <p class="text-gray-500 mb-6 text-sm leading-relaxed">Copy this URL to share the current dataset, visualization settings, and zoom state with others:</p>

  <div class="mb-6">
    <input type="text" class="form-control w-full font-mono text-sm text-gray-900 bg-gray-50 border-2 border-gray-300" value={shareableUrl} readonly onfocus={(e) => e.target.select()} />
  </div>
